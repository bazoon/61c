...
int is_valid_char(char c) {
    return isalnum(c) || c == ':' || c == '$' || c == 'x' ||  c  == '-';
}


int get_token(char *line, char *token, int last_pos) {
    int i = last_pos;
    int len = strlen(line);
    
    while(i < len && !is_valid_char(line[i])) {
      i++;
    }
    ...
    while(last_pos < len && !is_valid_char(line[last_pos])) {
      last_pos++;
    }

    strncpy(token, line + last_pos, i - last_pos);
    token[i - last_pos] = '\0';

    if (i >= len || last_pos >= len) {
        return -1;
    }
...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
  ...
  while(fgets(line, sizeof(line), input)) {
    ...
    instructions = strip_comments(line);
    
    args_count = 0;

    // no label
    if (strchr(instructions, ':') == NULL) {
        last_pos = get_token(instructions, name, last_pos);
        if (!name) {
            continue;
        }
        
        while(last_pos > 0)  {
            last_pos = get_token(instructions, args[args_count], last_pos);
            // printf("got %s:  \n last_pos: %d\n", args[args_count], last_pos);
            if (strlen(args[args_count]) > 0) {
                args_count++; 
            }
        ...
        last_pos = get_token(instructions, label, 0);
        int r = add_if_label(line_num, label, byte_offset, symtbl);
    ...
    if (args_count > 3) {
        raise_extra_arg_error(line_num, prev_pch);
        return -1;
    } else {
        if (name && strlen(name) > 0) {
            write_pass_one(output, name, args, args_count);
            line_num++;
            byte_offset += 4;
        }
    }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while(fgets(buf, sizeof(buf), input)) {
        last_pos = get_token(buf, token, last_pos);
        args_count = 0;
        ...
        while(last_pos > 0)  {
            // printf("%d %d %s ", last_pos, args_count, buf);
            last_pos = get_token(buf, args[args_count], last_pos);
        ...
        if(translate_inst(output, token, args, args_count, 0, symtbl, reltbl) < 0) {
            printf("Error detected on %s", token);
            return -1;
        }
    }
